...
class MeanQueue:
    ...
    def __init__(self):
        self.data_arr = make_array(MeanQueue.INITIAL_CAPACITY)
        self.capacity = MeanQueue.INITIAL_CAPACITY
        self.n = 0
        self.front_ind = None
...
class ArrayDeque:
    ...
    def last(self):
        if self.is_empty():
            raise Exception("Queue is empty")
        return self.data_arr[(self.front_end+self.n)%self.capacity]
    ...
    def dequeue_last(self):
        ...
        back_ind = (self.back_ind + 1) % self.capacity
        self.n -= 1
        if(self.is_empty()):
            self.front_ind = None
            back_ind = None
